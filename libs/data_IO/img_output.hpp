...
private:
  void WriteData(void *pix, bool is_complex, unsigned dataSize, const std::vector<unsigned> &shape, 
            ...
            const std::map<std::string, double> &parameters);
  /*
  void WriteImage(QSfMat &data, std::string label, std::vector<unsigned> &position=std::vector<unsigned>(), 
                      std::map<std::string, double> &parameters=std::map<std::string, double>());
  void WriteImage(QScMat &data, std::string label, std::vector<unsigned> &position=std::vector<unsigned>(), 
                         std::map<std::string, double> &parameters=std::map<std::string, double>());
  */
private:
