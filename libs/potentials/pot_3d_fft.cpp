...
C3DFFTPotential::C3DFFTPotential() :
		C3DPotential() {
}

C3DFFTPotential::C3DFFTPotential(const ConfigReaderPtr &configReader) :
		C3DPotential(configReader) {
...
void C3DFFTPotential::AddAtomNonPeriodic(std::vector<atom>::iterator &atom,
	...
	if ((xstart < m_nx) && (xend >= 0) && (ystart < m_ny) && (yend >= 0)) {
		...
		if (((int) iAtomZ + (int) zstart < (int) m_nslices)
				&& ((int) iAtomZ + (int) zend >= 0)) {
			// retrieve the pointer for this atom
			ComplexVector atPotPtr;
			GetAtomPotential3D(atom->Znum, m_tds ? 0 : atom->dw, nzSub, Nr,
					Nz_lut, atPotPtr);
			...
			unsigned iOffsetUpperLimit = Nr * (Nz_lut - 1);
			int iOffsetLowerLimit = -Nr * (Nz_lut - 1);
			...
			for (unsigned iax = xstart; iax <= xend; iax++) {
				unsigned idx = iax * m_ny + ystart;
				//complex_tt *potPtr = &(m_trans[iAtomZ+iaz0][idx]);
				complex_tt *potPtr = &(m_trans1[iAtomZ + zstart][ystart][iax]);
				...
				x2 *= x2;
				for (unsigned iay = ystart; iay <= yend; iay++) {
					// printf("iax=%d, iay=%d\n",iax,iay);
					...
					if (ir < Nr - 1) {
						...
						for (int iaz = zstart; iaz <= zend; iaz++) {
							float_tt potVal = 0;
							if (iOffsetZ < 0) {
								if (iOffsetZ > iOffsetLowerLimit) {
									// do the real part by linear interpolation in r-dimension:
#if Z_INTERPOLATION
									potVal = (1-ddz)*((1-ddr)*atPotPtr[ir-iOffsetZ+Nr].real()+ddr*atPotPtr[ir+1-iOffsetZ+Nr].real())+
									...
									if (atPotOffsPtr != NULL) {
										potVal += atoms[iatom].q*((1-ddz)*((1-ddr)*atPotOffsPtr[ir-iOffsetZ+Nr][0]+ddr*atPotOffsPtr[ir+1-iOffsetZ+Nr][0])+
												ddz *((1-ddr)*atPotOffsPtr[ir-iOffsetZ ][0]+ddr*atPotOffsPtr[ir+1-iOffsetZ ][0]));
									}
#endif // USE_Q_POT_OFFSETS
#else // Z_INTERPOLATION
									potVal = (1 - ddr)
											* atPotPtr[ir - iOffsetZ + Nr].real()
											+ ddr
													* atPotPtr[ir + 1 - iOffsetZ
															+ Nr].real();
#if USE_Q_POT_OFFSETS
									// add the charge-dependent potential offset
									if (atPotOffsPtr != NULL) {
										potVal += atoms[iatom].q*((1-ddr)*atPotOffsPtr[ir-iOffsetZ+Nr].real()+ddr*atPotOffsPtr[ir+1-iOffsetZ+Nr].real());
									}
#endif // USE_Q_POT_OFFSETS
								...
								if (iOffsetZ < iOffsetUpperLimit) {
									...
									ddz *((1-ddr)*atPotPtr[ir+iOffsetZ+Nr][0]+ddr*atPotPtr[ir+1+iOffsetZ+Nr][0]);
#if USE_Q_POT_OFFSETS
									// add the charge-dependent potential offset
									if (atPotOffsPtr != NULL) {
										potVal += atom->q*((1-ddz)*((1-ddr)*atPotOffsPtr[ir+iOffsetZ][0]+ddr*atPotOffsPtr[ir+1+iOffsetZ][0])+
												ddz *((1-ddr)*atPotOffsPtr[ir+iOffsetZ+Nr][0]+ddr*atPotOffsPtr[ir+1+iOffsetZ+Nr][0]));
									}
#endif // USE_Q_POT_OFFSETS
#else // Z_INTERPOLATION
									potVal =
											(1 - ddr)
													* atPotPtr[ir + iOffsetZ].real()
													+ ddr
															* atPotPtr[ir + 1
																	+ iOffsetZ].real();
#if USE_Q_POT_OFFSETS
...
//							*ptr += potVal; // ptr = potPtr = m_trans[...]
							m_trans1[iAtomZ + iaz][iAtomY + iay][iAtomX + iax] += potVal;
...
void C3DFFTPotential::AddAtomPeriodic(std::vector<atom>::iterator &atom,
	...
	unsigned iaz1 =
			iAtomZ + m_iRadZ >= m_nslices ? m_nslices - iAtomZ - 1 : m_iRadZ;
	...
	if ((iAtomZ + iaz0 < m_nslices) && (iAtomZ + iaz1 >= 0)) {
		// retrieve the pointer for this atom
		ComplexVector atPotPtr;
		GetAtomPotential3D(atom->Znum, m_tds ? 0 : atom->dw, nzSub, Nr, Nz_lut,
				atPotPtr);
#if USE_Q_POT_OFFSETS
		// retrieve the pointer to the array of charge-dependent potential offset
		// This function will return NULL; if the charge of this atom is zero:
		...
		for (unsigned iax = iax0; iax < iax1; iax++) {
			unsigned idx = ((iax + 2 * m_nx) % m_nx) * m_ny
					+ (iay0 + 2 * m_ny) % m_ny;
			complex_tt *potPtr = &(m_trans[iAtomZ + iaz0][idx]);
			...
			for (unsigned iay = iay0; iay < iay1;) {
				...
				if (ir < Nr - 1) {
					...
					float_tt dOffsZ = (iAtomZ + iaz0 - atomZ / m_sliceThickness)
							* nzSub;
#if Z_INTERPOLATION
...
//    temp.resize(nx*nz);
		tmp.resize(boost::extents[nx][nz]);
		...
		// make nzPerSlice odd:
		if (2.0 * (nzPerSlice >> 1) == nzPerSlice)
			nzPerSlice += 1;
		...
		if (scatPar[0][N_SF - 4] > scatPar[0][N_SF - 3]) {
			...
			if (m_printLevel > 1)
				printf(
						"getAtomPotential3D: set resolution of scattering factor to %g/A!\n",
						scatPar[0][N_SF - 4 - ix]);
...
//    memset((void*)&temp[0],0,nx*nz*sizeof(complex_tt));
		memset((void*) tmp.data(), 0, nx * nz * sizeof(complex_tt));
		...
		for (unsigned iz = 0; iz < nz; iz++) {
			...
			for (unsigned ix = 0; ix < nx; ix++) {
				...
				if (s2 < kmax2) {
					...
					float_tt f =
							seval(scatPar[0], scatPar[Znum], &splinb[0], &splinc[0], &splind[0], N_SF, sqrt(s2))
							* exp(-s2 * B * 0.25);
					...
					for (unsigned iy = 1; iy < nx; iy++) {
						...
						s3 = s3 * s3 + s2;
						if (s3 < kmax2) {
							f += 2* seval(scatPar[0], scatPar[Znum],&splinb[0], &splinc[0], &splind[0],
											N_SF, sqrt(s3))
									* exp(-s3 * B * 0.25);
						} else
							break;
					...
					tmp[ix][iz] = std::complex<float_tt>(f * cos(phase), // *zScale
					f * sin(phase)); // *zScale
		...
		ComplexArray2D out;
		out.resize(boost::extents[nx][nz]);
...
//						zScale += temp[ix + (iz + izOffset + iiz) * nx].real();
						zScale += out[ix][(iz + izOffset + iiz)].real()*(nx*nz);
				}
				if (zScale < 0)
					zScale = 0;
		...
		if (m_printLevel > 1)
			printf(
					"Created 3D (r-z) %d x %d potential array for Z=%d (B=%g, dkx=%g, dky=%g. dkz=%g,sps=%d)\n",
...
void C3DFFTPotential::GetAtomPotentialOffset3D(unsigned Znum, float_tt B,
...
#if !USE_REZ_SFACTS
	printf(
			"Using charged atoms only works with scattering factors by Rez et al!\n",
			Znum);
	exit(0);
#endif
	...
	if (m_offsetPot.size() == 0) {
		...
		if (m_printLevel > 1)
			printf(
					"Potential offset: will use %d sampling points per slice, total nz=%d (%d)\n",
					nzPerSlice, nz, nzPerSlice >> 1);
		...
		if (scatParOffs[0][N_SF - 4] > scatParOffs[0][N_SF - 3]) {
			...
			if (m_printLevel > 1)
				printf(
						"getAtomPotentialOffset3D: reduced angular range of scattering factor to %g/A!\n",
	...
	if (m_offsetPot.count(Znum) == 0) {
		// setup cubic spline interpolation:
		splinh(scatParOffs[0], scatParOffs[Znum], &splinb[0], &splinc[0],
				&splind[0], N_SF);
			...
			for (unsigned ix = 0; ix < nx; ix++) {
				...
				if (s2 < kmax2) {
					...
					float_tt f = seval(scatParOffs[0], scatParOffs[Znum],
							&splinb[0], &splinc[0], &splind[0], N_SF, sqrt(s2))
							* exp(-s2 * B * 0.25);
					// perform the qy-integration for qy <> 0:
					for (unsigned iy = 1; iy < nx; iy++) {
						float_tt s3 = dky * iy;
						...
						if (s3 < kmax2) {
							f += 2
									* seval(scatPar[0], scatPar[Znum],
											&splinb[0], &splinc[0], &splind[0],
											N_SF, sqrt(s3))
									* exp(-s3 * B * 0.25);
						} else
							break;
...
//          temp[ind3d][1] = f*sin(phase);        // *zScale
		...
		fftwf_plan plan = fftwf_plan_dft_2d(nz, nx, ptr, ptr, FFTW_BACKWARD,
				FFTW_ESTIMATE);
		fftwf_execute(plan);
		fftwf_destroy_plan(plan);
#else
		...
		if (m_printLevel > 1)
			printf(
					"Created 3D (r-z) %d x %d potential offset array for Z=%d (B=%g, dkx=%g, dky=%g. dkz=%g,sps=%d)\n",
